...
bool runPauseScreen(SDL_Screen &Scene, SDL_Audio &Audio) {
		...
		switch(event.type) {
			...
			case SDL_MOUSEBUTTONDOWN: {
				...
				if (SDL_BUTTON_LEFT == event.button.button) {
					...
					} else if (coord_click_x > 0 && coord_click_x < 0 + 240 && coord_click_y >= 100 && coord_click_y < 100 + 30) {
						unloadCurrentLevel(Audio);
						if (Scene.isFullscreen())
							Scene.showCursor(false);
	...
	Scene.drawRectangle(0, 100, 240, 30, CLR_GRY);
	
	char buffer[50];
	snprintf(buffer, 50, "SFX: %d", (int)std::round(((float)Mix_Volume(-1, -1)/128)*100));
	Scene.drawText(buffer, 0, 30, 30);
	
	snprintf(buffer, 50, "Music: %d", (int)std::round(((float)Mix_VolumeMusic(-1)/128)*100));
	Scene.drawText(buffer, 0, 60, 30);
	
	snprintf(buffer, 50, "Exit level");
	Scene.drawText(buffer, 0, 100, 30);
	Scene.finishDrawing();
	return true;
}